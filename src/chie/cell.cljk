...
#?(:clj
   (defn- actor-dir
     "., resolved from this namespace's classpath location so the cell
     runs from any cwd (the cell-runner's contract)."
     []
     (-> (io/resource "chie/cell.cljc") io/file .getParentFile .getParentFile .getParentFile)))
...
#?(:clj
   (defn fire
     ...
     ([] (fire nil))
     ([log-path]
      (let [seed (io/file (actor-dir) "data" "seed.edn")

(ns chie.methods.social
  "social.cljc — chie DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from chie's own
...
(defn draft-observation-post
  "A dry-run observation post over chie's own domain (AI-ecosystem knowledge-graph mirror — the AI sibling of the power-mirror lineage (tsumugi/keizu/kabuto/kanjō/kosatsu). Weaves AI labs /…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "chie R0: live social posting is Council Lv6+ + operator + "
